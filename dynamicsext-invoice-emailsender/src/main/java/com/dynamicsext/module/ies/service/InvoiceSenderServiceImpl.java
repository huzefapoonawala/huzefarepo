			...
			for (TransactionVO t : transactions) {
				if (StringUtils.isNotBlank(t.getEmailAddress()) && t.getIsOptForEmail()) {
					...
					if (toPreviewFile.isFile() || previewedFile.isFile()) {
						...
						List<TransactionEntryVO> transactionEntries = jdbcTemplate.query("select i.ItemLookupCode, concat(i.Description, case when t.Comment is not null and len(t.Comment) > 0 then concat('<br>&nbsp;(',t.Comment,')') else '' end, case when r.Code is not null and len(r.Code) > 0 then concat('<br>&nbsp;(Return code: ',r.Code,')') else '' end) as description, t.Quantity, t.Price, t.Quantity*t.Price as extPrice from TransactionEntry t inner join Item i on t.ItemID = i.ID left join ReasonCode r on t.ReturnReasonCodeID = r.ID where TransactionNumber = ?;", new BeanPropertyRowMapper<TransactionEntryVO>(TransactionEntryVO.class), t.getTransactionNumber());
